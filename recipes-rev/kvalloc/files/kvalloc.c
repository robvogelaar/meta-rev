...
/*
This kernel module also demos the common technique of having a global "(device)
context"
...
//------------------------------------------
/*
vmalloc_test Read callback: just display the last # bytes attempting to
allocate...
*/
static int vmalloc_procread(char *buf, char **start, off_t offset, int count,
                            int *eof, void *data) {
  ...
  *eof = 1;
  n = sprintf(
      ...
      DRVNAME, pstCtx->kvalloc, pstCtx->kvalloc / 1024,
...
static int vmalloc_procwrite(struct file *file, const char __user *buffer,
  ...
  MSG("Successfully allocated via vmalloc %u bytes (%u Kb, %u MB) now to "
      "location 0x%08x (will vfree..)\n",
      pstCtx->kvalloc, pstCtx->kvalloc / 1024, pstCtx->kvalloc / (1024 * 1024),
      (u32)pstCtx->vm);
...
//------------------------------------------
/*
kmalloc_test Read callback: just display the last # bytes attempting to
allocate...
*/
static int kmalloc_procread(char *buf, char **start, off_t offset, int count,
                            int *eof, void *data) {
  PST_CTX pstCtx = (PST_CTX)data;
  int n;
...
static int kmalloc_procwrite(struct file *file, const char __user *buffer,
  ...
  MSG("Successfully allocated via kmalloc %u bytes (%u Kb, %u MB) now to "
      "location 0x%08x (will kfree..)\n",
      pstCtx->kvalloc, pstCtx->kvalloc / 1024, pstCtx->kvalloc / (1024 * 1024),
      (u32)pstCtx->vm);
...
static int __init vmall_init_module(void) {
  ...
  static struct file_operations k_proc_fops = {
      .read = kmalloc_procread, .write = kmalloc_procwrite,
  };

  // res = create_proc_entry (gpstCtx->kname, 0644, /* mode */ NULL);
  res = proc_create(gpstCtx->kname, 0644, /* mode */ NULL, &k_proc_fops);
  if (!res) {
  ...
  static struct file_operations v_proc_fops = {
      .read = vmalloc_procread, .write = vmalloc_procwrite,
  };

  // res = create_proc_entry (gpstCtx->vname, 0644, /* mode */ NULL);
  res = proc_create(gpstCtx->vname, 0644, /* mode */ NULL, &v_proc_fops);
  if (!res) {
