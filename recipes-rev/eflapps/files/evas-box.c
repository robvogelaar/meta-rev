...
static Eina_Bool _animate_scene(void *data) {
  ...
  if (flip & 0x1) {
    evas_object_box_layout_set(d.box, evas_object_box_layout_horizontal, NULL,
                               NULL);
  } else {
    evas_object_box_layout_set(d.box, evas_object_box_layout_vertical, NULL,
                               NULL);
  }
...
static void /* custom 'diagonal' layout */
    _custom_layout(Evas_Object *o, Evas_Object_Box_Data *p,
 ...
 * the key for help */
static void _on_keydown(void *data EINA_UNUSED, Evas *evas EINA_UNUSED,
  ...
  if (strcmp(ev->key, "a") == 0) {
    ...
    fprintf(stdout,
            "Applying new alignment values (%.1f, %.1f)"
            " on the box\n",
            h, v);
    return;
  }
  ...
  if (strcmp(ev->key, "p") == 0) {
    ...
    fprintf(stdout,
            "Applying new padding values (%d, %d)"
            " on the box\n",
            h, v);
    return;
  }
  ...
  if (strcmp(ev->key, "3") == 0) {
    evas_object_box_layout_set(
        d.box, evas_object_box_layout_homogeneous_horizontal, NULL, NULL);
  ...
  if (strcmp(ev->key, "5") == 0) {
    evas_object_box_layout_set(
        d.box, evas_object_box_layout_homogeneous_max_size_horizontal, NULL,
        NULL);
  ...
  if (strcmp(ev->key, "6") == 0) {
    evas_object_box_layout_set(
        d.box, evas_object_box_layout_homogeneous_max_size_vertical, NULL,
        NULL);
  ...
  if (strcmp(ev->key, "7") == 0) {
    evas_object_box_layout_set(d.box, evas_object_box_layout_flow_horizontal,
                               NULL, NULL);
  ...
  if (strcmp(ev->key, "8") == 0) {
    evas_object_box_layout_set(d.box, evas_object_box_layout_flow_vertical,
                               NULL, NULL);
  ...
  if (strcmp(ev->key, "9") == 0) {
    evas_object_box_layout_set(d.box, evas_object_box_layout_stack, NULL, NULL);
...
static void _on_delete(Ecore_Evas *ee EINA_UNUSED) { ecore_main_loop_quit(); }
...
int main(void) {
  ...
  evas_object_event_callback_add(d.bg, EVAS_CALLBACK_KEY_DOWN, _on_keydown,
                                 NULL);
  ...
  for (i = 1; i <= 5; i++) {
    ...
    evas_object_color_set(o, rand() % 256, rand() % 256, rand() % 256, 255);
    evas_object_show(o);
  ...
  ecore_timer_add(0.5, _animate_scene, NULL);
